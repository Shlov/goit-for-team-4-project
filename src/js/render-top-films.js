...
export function renderGalleryFilms(cards) {
  const markup = cards.map(card => {
    ...
    <button data-id="${id}" class="button-youtube"></button>
      ...
      <p class="card__info lng-cardInfo"> ${change(genresInfo, genre_ids)} | ${release_date.split('-')[0] || 'Coming soon'}<span class="card__rating"> ${vote_average} </span></p></div>
  </li>` }).join('')
